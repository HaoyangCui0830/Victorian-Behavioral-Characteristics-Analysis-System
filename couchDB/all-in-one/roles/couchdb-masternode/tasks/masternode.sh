...
echo "== Set variables =="
declare -a nodes=(172.26.132.170 172.26.133.140 172.26.131.248)
export masternode=172.26.132.170
export user=admin
...
sudo docker run -d -p 5984:5984 -p 5986:5986 -p 4369:4369 -p 9100:9100 --name=mastercouchdb couchdb:2.3.0
sleep 3

sudo docker exec mastercouchdb bash -c "echo \"-setcookie couchdb_cluster\" >> /opt/couchdb/etc/vm.args"
